...
function sectionLabel(s: SectionType) {
  ...
  if (s === "latest") return "Latest research papers (Slider)";
  return "Archive (Slider)";
}

function folderForSection(page: string, s: SectionType) {
  return `epc/uploads/${page}/${s}`;
}

export default function AdminSecurityDiplomacy() {
  const API_BASE = import.meta.env.VITE_API_BASE_URL || "http://localhost:5000";
  const page = "security-diplomacy";
  ...
  const [form, setForm] = useState({ ...emptyForm, page });
  ...
  function resetForm() {
    setEditId(null);
    setForm({ ...emptyForm, page, section: activeSection });
    setUploadPct(0);
  }
  ...
  async function onPickImage(file: File) {
    ...
    if (!allowed.includes(file.type)) return setError("Only JPG, PNG, WEBP images are allowed");
    if (file.size > 10 * 1024 * 1024) return setError("Max image size is 10MB");
    ...
    try {
      const { secureUrl } = await uploadToCloudinarySigned(file, {
        folder: folderForSection(page, form.section),
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-heading text-2xl font-bold text-brand-ink">Security & Diplomacy Content</h1>
          <p className="mt-1 text-sm text-slate-600">
            Manage Featured / Latest Research Papers / Archive. Only Published items show on the public page.
          </p>
        </div>
        <button
          onClick={load}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <div>
            ...
            >
              ...
              <option value="latest">Latest research papers (Slider)</option>
              <option value="archive">Archive (Slider)</option>
            </select>
          </div>
